...
def get_more_topic(query, epoch, topic_dir):
    ...
    # 获取返回的总页数
    base_url = (
        "https://m.weibo.cn/api/container/getIndex?containerid=100103type%3D38%26q%3D"
        + str(query)
    ...
    while page_count <= page:
        ...
        print(
            f'[{time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())}]  Crawling Topic. Page {page_count} of keyword {query}'
        )
        try:
    ...
    # 结果写入文件
    with open(
        topic_dir + "Topics_" + str(epoch) + ".csv", "a", encoding="utf-8", newline=""
    ) as f:
        writer = csv.writer(f)
    ...
    # 获取元素输出
    print(
        f'[{time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())}]  Finished Crawling Topic. Get {len(topic_list)} new topic for keyword {query}'
    )

...
def get_query_info(wd, writer, logger, since_date=None):
    ...
    # 爬取检索页面下热门栏的页面
    base_url = (
        "https://m.weibo.cn/api/container/getIndex?containerid=100103type%3D60%26q%3D"
        + quote(wd)
        + "%26t%3D0&page_type=searchall"
    )
    # 计算可获取的总页数
    page = load_config()["page_num"] - 1
    if page == None:
    ...
    while page_count <= page:
        ...
        logger.info(f"Word {wd} Page {page_count}: {this_url}")
        print(
            f'[{time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())}]  Word {wd} Page {page_count}.'
        )
        try:

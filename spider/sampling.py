...
def sampling():
    with open("./data/search_result_新冠.csv", "r", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        raw_list = list(set([row["bw_id"] for row in reader]))

    df = pd.read_csv("./data/repost_Relationship_新冠.csv", header=0)
    ...
    result.to_csv("./data/sample.csv", index=False)
